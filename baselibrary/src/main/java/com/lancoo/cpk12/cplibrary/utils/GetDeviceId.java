...
/**
 * 获取设备唯一标识，通过 imie + macAddress
 * 如果这两个都没有的化自动生成uuid。然后对标识进行md5加密
 * 将标识保存到了Android/data/.devices/.DEVICES 文件中了
 * 需要 READ_PHONE_STATE 这个权限来获取imie
 */

public class GetDeviceId {
    private static boolean isDebug = false;
    private static final String TAG = GetDeviceId.class.getSimpleName();
    //保存文件的路径
    private static final String CACHE_IMAGE_DIR = "Android/data/.devices";
    //保存的文件 采用隐藏文件的形式进行保存
    ...
    private static File getDevicesDir(Context context) {
        ...
        if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
            ...
            File cropdir = new File(context.getFilesDir(),
                    "/.devices");
            if (!cropdir.exists()) {
                cropdir.mkdirs();
            }
