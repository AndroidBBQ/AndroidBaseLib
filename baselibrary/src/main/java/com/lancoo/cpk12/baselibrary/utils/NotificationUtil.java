...
public class NotificationUtil {
    ...
    public static void initChannel(Context context) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            String channelId = CHANNEL_MESSAGE;
...
//        channel.setVibrationPattern(new long[]{1000, 2000, 1000,3000});
    ...
    /**
     * TODO: 发送通知（刷新前面的通知），默认发送的是home频道
     *
     * @param context
    ...
    public static void show(Context context, String contentTitle, String contentText, Class<?> cls) {
        show(context, contentTitle, contentText, null, 0, CHANNEL_MESSAGE, cls);
    }
    ...
    public static void show(Context context, String contentTitle, String contentText, RemoteViews views, Class<?> cls) {
        show(context, contentTitle, contentText, views, 0, CHANNEL_MESSAGE, cls);
    }
    ...
    public static void showMuch(Context context, String contentTitle, String contentText, Class<?> cls) {
        show(context, contentTitle, contentText, null, ++notifyId, CHANNEL_MESSAGE, cls);
    }
    ...
    public static void showMuch(Context context, String contentTitle, String contentText, RemoteViews views, Class<?> cls) {
        show(context, contentTitle, contentText, views, ++notifyId, CHANNEL_MESSAGE, cls);
    }
    ...
    public static void show(Context context, int largeIcon,
        ...
        builder.setLargeIcon(BitmapFactory.decodeResource(context.getResources(), largeIcon == 0 ? R.drawable.cpbase_ic_cloud : largeIcon)) //设置自动收报机和通知中显示的大图标。
                .setSmallIcon(smallIcon == 0 ? R.drawable.cpbase_ic_cloud : smallIcon) // 小图标
...
//                .setTimeoutAfter(3000)//指定取消此通知的时间（如果尚未取消）。
     ...
     * @return
     */
    public static NotificationCompat.Builder getMessageNotificataion(Context context, int smallIcon, int largeIcon, Class<?> cls) {
      /*  //创建频道，如果没有频道会创建频道，有的话不会再次创建
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            String channelId = CHANNEL_MESSAGE;
            String channelName = "消息";
        ...
        //判断应用通知是否打开
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            if (!openNotificationChannel(context, manager, CHANNEL_MESSAGE)) {
                return null;
            }
        }
        //创建 NEW_MESSAGE 渠道通知栏  在API级别26.1.0中推荐使用此构造函数 Builder(context, 渠道名)
        NotificationCompat.Builder builder = (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) ? new NotificationCompat.Builder(context, CHANNEL_MESSAGE) : new NotificationCompat.Builder(context);
        builder.setLargeIcon(BitmapFactory.decodeResource(context.getResources(), largeIcon == 0 ? R.drawable.cpbase_ic_cloud : largeIcon)) //设置自动收报机和通知中显示的大图标。
                .setSmallIcon(smallIcon == 0 ? R.drawable.cpbase_ic_cloud : smallIcon) // 小图标
                .setWhen(System.currentTimeMillis()) // 设置通知发送的时间戳
                .setShowWhen(true)//设置是否显示时间戳
...
//                .setDefaults(Notification.PRIORITY_HIGH)// 设置默认的提示音、振动方式、灯光等 使用的默认通知选项
     ...
     * @return
     */
    public static NotificationCompat.Builder getMessageNotificataion(Context context, Class<?> cls) {
        return getMessageNotificataion(context, R.drawable.cpbase_ic_cloud, R.drawable.cpbase_ic_cloud, cls);
    }
